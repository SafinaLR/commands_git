...
```
$ git log # просмотреть историю коммитов в обратном хронологическом порядке — последние коммиты оказываются первыми сверху
```
...
```
$ git push -u origin main # выдается при **первом обращении**. Если команда приведёт к ошибке, попробуйте 
